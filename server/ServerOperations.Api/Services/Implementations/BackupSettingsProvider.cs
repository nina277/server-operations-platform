...
/// <summary>
/// バックアップ設定と秘密値の取得。秘密値は都度復号し、保持・ログ出力しない。
/// </summary>
    ...
    IDataProtectionProvider dataProtectionProvider) : IBackupSettingsProvider
{
    ...
    public Task<string?> GetAccessKeyAsync(CancellationToken ct = default) =>
        UnprotectAsync("backup-access-key", ct);

    public Task<string?> GetSecretKeyAsync(CancellationToken ct = default) =>
        UnprotectAsync("backup-secret-key", ct);
